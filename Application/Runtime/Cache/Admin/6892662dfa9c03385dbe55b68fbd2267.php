...
<div class="m-b-md">
    <h2 class="m-b-none">货品管理/货品编辑</h2>
</div>
<form class="form-horizontal" id="form" action="" method="post">
    <div class="panel panel-default">
        <div class="panel-body">
            <?php if(is_array($attrData)): foreach($attrData as $key=>$v): ?><div class="form-group">
                <label for="" class="col-sm-2 control-label"><?php echo ($v['attr_name']); ?></label>
                <div class="col-sm-3">
                    <select class="form-control" name="combine[]">
                        <?php if(is_array($v['select'])): foreach($v['select'] as $key=>$vv): ?><option value="<?php echo ($vv['ga_id']); ?>" <?php foreach($oldData['ga_id'] as $k=>$v) {if($vv['ga_id']==$v){echo 'selected';}} ?>><?php echo ($vv['goods_attr_name']); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="number" name="inventory" value="<?php echo ($oldData['inventory']); ?>" required min="0">
                </div>
            </div>
            <div class="form-group">
                <label for="" class="col-sm-2 control-label">货号</label>
                <div class="col-sm-9">
                    <input type="text" name="goods_number" value="<?php echo ($oldData['goods_number']); ?>" placeholder="请输入货号" required>
                </div>
            </div>
        </div>
    </div>
    <button class="btn btn-primary col-sm-4 col-sm-offset-1" type="submit">确定提交</button>
</form>
</div>
